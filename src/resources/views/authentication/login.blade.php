...
@section('content')
	<form class="form-signin" method="POST" action="{{ route('attemptLogin') }}">
		...
		<div class="text-center mb-4">
            ...
            <h2 class="h2 mb-3 font-weight-normal">{{ trans('wire::wire::authentication.sign_in') }}</h2>
            <h4 class="h4 mb-3 font-weight-normal">{{ trans('wire::wire::heading.asarsoft_authentication') }}</h4>
		</div>
        ...
        <div class="form-label-group">
			<input type="text" id="username_or_email" name="username_or_email" class="form-control" placeholder="{{ trans('wire::wire::authentication.username_or_email') }}" required autofocus value="{{ old('username_or_email') }}">
			<label for="username_or_email">{{ trans('wire::wire::authentication.username_or_email') }}</label>
		...
		<div class="form-label-group">
			...
			<label for="password">{{ trans('wire::wire::authentication.password') }}</label>
		...
		<p class="mt-5 mb-3 text-muted text-center">&copy; {{ now()->year }} <b>{{ trans('wire::wire::app.name') }}</b></p>
	</form>
@endsection
